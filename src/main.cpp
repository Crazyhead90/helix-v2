...
#if defined(NDEBUG)
#error "Helix cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uhlix) are considered zero fee (for relaying and mining)
...
int GetBaseVersion (int nVersion) {
    return nVersion & 255;
}
...
bool GetCoinAge(const CTransaction& tx, const unsigned int nTxTime, uint64_t& nCoinAge)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        // Read block header
	if (fDebug) LogPrintf("GetCoinAge: do GetBlockHeader()\n");
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock) {
    //Check to see if the zHLIX is properly signed
    if (pindex->nHeight > Params().Zerocoin_LastOldParams()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zHLIX spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zHLIX without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zHLIX spend with serial %s is already in block %d\n", __func__,
    ...
    if (!spend.HasValidSerial(paramsToUse))
        return error("%s : zHLIX spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zHLIX spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zhlix");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zHLIX mints are not already known
            if (tx.IsZerocoinMint()) {
                for (auto& out : tx.vout) {
                    if (!out.IsZerocoinMint())
                        continue;
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos)
{
    ...
    // Check the header
    if (block.IsProofOfWork() && block.nVersion > 3) {
      if (!CheckAuxPowProofOfWork(block,Params().GetAuxpowChainId()))
...
int64_t GetBlockValue(int nHeight) {
  if (nHeight == 0) {
    return 5000000 * COIN; // this reward is for block 1 (prev is block 0)
...
BoostBigNum get_ssf (const CBlockIndex* pindex) {
  ...
  for (int i=0; i<90; i++) { // use at most a year's worth of history
    if (fDebug) LogPrintf("get_ssf algo %d period %d\n",pprev_algo ? pprev_algo->GetBlockAlgo() : -2,i);
    pprev_algo = get_pprev_algo(pprev_algo,-1);
    if (!pprev_algo) {
      if (fDebug) LogPrintf("!pprev_algo\n");
      break;
    }
    BoostBigNum hashes = pprev_algo->GetBlockWorkBoost();
    if (fDebug) LogPrintf("start block %d hashes %lld\n",pprev_algo->nHeight,hashes.convert_to<int64_t>());
    ...
    for (int j=0; j<nSSF-1; j++) {  // nSSF blocks = 24 hours, using only blocks from the same algo as the target block
      pprev_algo = get_pprev_algo(pprev_algo,-1);
      if (!pprev_algo) {
	if (fDebug) LogPrintf("j=%d !pprev_algo\n");
	hashes = BoostBigNum(0);
	break;
      }
      BoostBigNum hashes_add = pprev_algo->GetBlockWorkBoost();
      if (fDebug) LogPrintf("j=%d block=%d hashes_add %lld\n",j,pprev_algo->nHeight,hashes_add.convert_to<int64_t>());
  ...
  if (hashes_peak > BoostBigNum(0) && hashes_cur != hashes_peak) {
    ...
    if (scalingFactor > std::numeric_limits<uint64_t>::max())
      scalingFactor = BoostBigNum(std::numeric_limits<uint64_t>::max());
  }
  return scalingFactor;
}

int64_t GetBlockValue(CBlockIndex* pindexPrev, int algo, bool scale)
{
  int nHeightPrev = pindexPrev->nHeight;
  bool onCEMNow = pindexPrev->onCEMNext();
  
  if (!onCEMNow || algo==ALGO_POS) {
    return GetBlockValue(nHeightPrev);
  }

  if (fDebug) LogPrintf("GetBlockValue() prevHeight=%d algo=%d\n",pindexPrev->nHeight,algo);
  
  int64_t emitted;

  CBlockIndex * pprev_algo = pindexPrev;
  if (pindexPrev->GetBlockAlgo() != algo) {
    pprev_algo = get_pprev_algo(pindexPrev,algo);
  }

  if (pprev_algo) { // normalize the emission
    if (fDebug) LogPrintf("have pprev_algo, so use it's moneysupply\n");
      emitted = 30 * pprev_algo->nMoneySupplyAlgo;
  }
  else {
    if (fDebug) LogPrintf("don't have pprev_algo so use mpow correction\n");
    emitted = 30 * get_mpow_ms_correction(pindexPrev,algo);
  }

  BoostBigNum scalingFactor = BoostBigNum(0);
  if (scale && pprev_algo) {
    if (fDebug) LogPrintf("scale && pprev_algo\n");
    scalingFactor = pprev_algo->subsidyScalingFactor;
    if (!scalingFactor.convert_to<uint32_t>()) { // find the key block and recalculate
      if (fDebug) LogPrintf("find key block and recalculate SSF\n");
      do {
	if (pprev_algo->update_ssf_next()) {
	  if (fDebug) LogPrintf("height %d update_ssf_next\n",pprev_algo->nHeight);
	  scalingFactor = get_ssf(pprev_algo);
	  pprev_algo->subsidyScalingFactor = scalingFactor;
	  break;
	}
	pprev_algo = get_pprev_algo(pprev_algo,-1);
      } while (pprev_algo && pprev_algo->onCEM());
    }
  }
  else {
  ...
  if (fDebug) LogPrintf("emitted = %lld, scalingFactor = %ld\n",emitted,scalingFactor.convert_to<uint32_t>());

  if (emitted < 14990000*COIN) {
    baseSubsidy = 111*COIN;
  }
  else if (emitted < 21521000*COIN) {
    baseSubsidy = 15*COIN;
  }
  else if (emitted < 35712200*COIN) {
    baseSubsidy = 13.5*COIN;
  }
  else if (emitted < 47065160*COIN) {
    baseSubsidy = 10.8*COIN;
  }
  else if (emitted < 500000150*COIN) {
    baseSubsidy = 8.1*COIN;
  }
  else if (emitted < 500000158.4*COIN) {
    baseSubsidy = 4.2*COIN;
  }

  if (fDebug) LogPrintf("baseSubsidy = %lld\n",baseSubsidy);

  if (!scalingFactor.convert_to<uint32_t>()) return baseSubsidy;
  return baseSubsidy - ((BoostBigNum(baseSubsidy)*BoostBigNum(100000000))/scalingFactor).convert_to<uint32_t>() * 2 / 3; // lower by 2/3 max
...
bool IsInitialBlockDownload()
{
    ...
    bool state = (chainActive.Height() < pindexBestHeader->nHeight - 24 * 6 ||
                  pindexBestHeader->GetBlockTime() < GetTime() - 6 * 60 * 60 && Params().NetworkID() != CBaseChainParams::TESTNET) && pindexBestHeader->nHeight >= Params().MultiPow_StartHeight(); // ~144 blocks behind -> 2 x fork detection time
    if (!state)
        lockIBDState = true;
    return state;
}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Helix
...
{
    RenameThread("helix-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZHLIXMinted()
{
...
void RecalculateZHLIXSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zHLIX supply
...
bool RecalculateHLIXSupply(int nHeightStart)
{
...
{
    // Helix: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZHLIXSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
    BlockToZerocoinMintList(block, listMints);
    std::list<libzerocoin::CoinDenomination> listSpends = ZerocoinSpendListFromBlock(block);

    if (fDebug) LogPrintf("UpdateZHLIXSupply: do GetBlockHeader()\n");
...
int64_t get_mpow_ms_correction(const CBlockIndex* p, int algo) {
  while (p) {
    if (!p->onFork()) {
      if (p->nHeight == 0) {
	return 0;
      ...
      if (algo < 3)
	return p->nMoneySupplyAlgo/30; // 1/10 blocks are PoW, 1/3 of those are for a PoW algo
      return p->nMoneySupplyAlgo-p->nMoneySupplyAlgo/10;
    }
    p = p->pprev;
  }
  return 0;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().MultiPow_StartHeight() && block.IsProofOfStake())
	return state.DoS(100, error("ConnectBlock(): PoW revival fork must start with a PoW block"), REJECT_INVALID, "PoS-on-PoW-revival");

    if (pindex->nHeight <= Params().LAST_POW_BLOCK() && block.IsProofOfStake())
        return state.DoS(100, error("ConnectBlock() : PoS period not active"),
            REJECT_INVALID, "PoS-early");

    if (pindex->nHeight > Params().LAST_POW_BLOCK() && pindex->nHeight < Params().MultiPow_StartHeight() && block.IsProofOfWork())
        return state.DoS(100, error("ConnectBlock() : PoW period ended"),
	REJECT_INVALID, "PoW-ended");

    if (pindex->nHeight >= Params().MultiPow_StartHeight() && GetBaseVersion(pindex->nVersion) < 5)
	return state.DoS(100, error("ConnectBlock() : MultiPow period blocks must have core nVersion >= 5"));

    bool onForkNow = pindex->onFork();
    bool onCEMNow = pindex->onCEM();
    
    // Check SSF
    if (onCEMNow && pindex->GetBlockAlgo()!=ALGO_POS) {
      CBlockIndex * pprev_algo = pindex;
      if (pindex->update_ssf_next()) {
	for (int i=0; i<nSSF; i++) {
	  pprev_algo = get_pprev_algo(pprev_algo,-1);
	  if (!pprev_algo) break;
	  if (pprev_algo->update_ssf_next()) {
	    if (i != nSSF-1) {
	      if (fDebug) LogPrintf("marked with update ssf flag, but not at right time: i=%d\n",i);
	...
	for (int i=0; i<nSSF; i++) {
	  ...
	  if (pprev_algo->update_ssf_next()) {
	    if (i == nSSF-1) {
	      if (fDebug) LogPrintf("Should be marked with update ssf flag\n");
	      return false; //make sure the SSF update block happens every nSSF blocks
	    }
	  }
	}
    ...
    if (!fJustCheck && onCEMNow) { // set scaling factor
      if (pindex->update_ssf_next()) {
    ...
    UpdateZHLIXSupply(block, pindex);
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zHLIXSpent: %s\n",
...
//              FormatMoney(nFees), FormatMoney(pindex->nMint), FormatMoney(nAmountZerocoinSpent));
    ...
    CAmount nExpectedMint = GetBlockValue(pindex->pprev,pindex->GetBlockAlgo());
    ...
    if (onForkNow) {
      if (fDebug) LogPrintf("onForkNow with nValueOut-nValueIn = %lld\n",nValueOut-nValueIn);
      if (pprev_algo) {
	if (fDebug) LogPrintf("have pprev_algo (height %d) so set nMoneySupplyAlgo = %lld\n",pprev_algo->nHeight,pprev_algo->nMoneySupplyAlgo + nValueOut - nValueIn);
	pindex->nMoneySupplyAlgo = pprev_algo->nMoneySupplyAlgo + nValueOut - nValueIn;
      }
      else {
	int64_t ms_correction = get_mpow_ms_correction(pindex->pprev,pindex->GetBlockAlgo());
	if (fDebug) LogPrintf("ms_correction = %lld\n",ms_correction);
	pindex->nMoneySupplyAlgo = ms_correction + nValueOut - nValueIn;
      }
    }
    else {
      if (pindex->pprev) {
	if (fDebug) LogPrintf("set nMoneySupplyAlgo to %lld\n",pindex->pprev->nMoneySupplyAlgo + nValueOut - nValueIn);
	pindex->nMoneySupplyAlgo = pindex->pprev->nMoneySupplyAlgo + nValueOut - nValueIn;
      }
      else {
	pindex->nMoneySupplyAlgo = 0;
      }
    }
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert HLIX to zHLIX
    ...
    LogPrintf("UpdateTip: new best=%s  height=%d  log2_work=%.8g  tx=%lu  date=%s progress=%f  cache=%u moneySupplyAlgo=%lld\n",
        chainActive.Tip()->GetBlockHash().ToString(), chainActive.Height(), log(chainActive.Tip()->nChainWork.getdouble()) / log(2.0), (unsigned long)chainActive.Tip()->nChainTx,
        DateTimeStrFormat("%Y-%m-%d %H:%M:%S", chainActive.Tip()->GetBlockTime()),
	      Checkpoints::GuessVerificationProgress(chainActive.Tip()), (unsigned int)pcoinsTip->GetCacheSize(),
	      chainActive.Tip()->nMoneySupplyAlgo);
...
CBlockIndex* AddToBlockIndex(const CBlock& block)
{
    ...
    if (miPrev != mapBlockIndex.end()) {
      if (fDebug) LogPrintf("miPrev != mapBlockIndex.end()\n");
	...
	if (fDebug) LogPrintf("compute stake entropy bit\n");
	
        // ppcoin: compute stake entropy bit for stake modifier
        if (!pindexNew->SetStakeEntropyBit(pindexNew->GetStakeEntropyBit()))
            LogPrintf("AddToBlockIndex() : SetStakeEntropyBit() failed \n");

	if (fDebug) LogPrintf("record proof-of-stake hash value\n");
	...
	if (fDebug) LogPrintf("compute stake modifier\n");
	...
	if (fDebug) LogPrintf("did ComputeNextStakeModifier\n");
	...
	if (fDebug) LogPrintf("did CheckStakeModifierCheckpoints\n");
    }
    pindexNew->nChainWork = (pindexNew->pprev ? pindexNew->pprev->nChainWork : 0) + GetBlockProof(*pindexNew);
    if (block.IsAuxpow()) {
      pindexNew->pauxpow = block.pauxpow;
...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, bool fCheckPOW)
{
  ...
  if (fCheckPOW && (block.nVersion & 255 > 4) && (block.IsAuxpow())) {
    if (!CheckAuxPowProofOfWork(block, Params().GetAuxpowChainId())) {
      return state.DoS(50, error("CheckBlockHeader() : auxpow proof of work failed"), REJECT_INVALID, "high-hash");
    }
  }
  else if (fCheckPOW && (block.nVersion > 3) && !CheckProofOfWork(block.GetPoWHash(), block.nBits, block.GetAlgo())) {
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    unsigned int nBitsRequired = GetNextWorkRequired(pindexPrev, block.GetAlgo());

    if (block.IsProofOfWork() && pindexPrev->nHeight + 1 > chainParams.LAST_POW_BLOCK() && pindexPrev->nHeight + 1 < chainParams.MultiPow_StartHeight())
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
    ...
    if (nHeight > Params().Zerocoin_StartHeight()) {
        ...
        for (const CTransaction& tx : block.vtx) {
            ...
            // double check that there are no double spent zHLIX spends in this block
            if (tx.IsZerocoinSpend()) {
                for (const CTxIn txIn : tx.vin) {
                    if (txIn.scriptSig.IsZerocoinSpend()) {
                        libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                        if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                            return state.DoS(100, error("%s : Double spending of zHLIX serial %s in block\n Block: %s",
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if ((block.IsAuxpow() || block.GetAlgo()) != ALGO_QUARK && pindexPrev->nHeight + 1 < Params().MultiPow_StartHeight()) {
      return state.DoS(100, error("AcceptBlock(): new block format required fork activation"), REJECT_INVALID, "bad-version-fork");
    }
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zHLIX mints and %d zHLIX spends\n", __func__, nMints, nSpends);
    ...
    if (!fLiteMode) {
	    ...
	    LogPrintf("requestedmasternodeassets > masternode_sync_list\n");
...
bool TestBlockValidity(CValidationState& state, const CBlock& block, CBlockIndex* const pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (fDebug) LogPrintf("TestBlockValidity nBits %08x\n",block.nBits);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        if (pfrom->cleanSubVer == "/Helix Core:1.0.3/" ||
            pfrom->cleanSubVer == "/Helix Core:1.0.3.1/" ||
            pfrom->cleanSubVer == "/Helix Core:1.0.3.2/") {
            LOCK(cs_main);
        ...
        // Helix: We use certain sporks during IBD, so check to see if they are
