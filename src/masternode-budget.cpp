...
bool IsBudgetCollateralValid(uint256 nTxCollateralHash, uint256 nExpectedHash, std::string& strError, int64_t& nTime, int& nConf, bool fBudgetFinalization)
{
    ...
    for (const CTxOut o : txCollateral.vout) {
        ...
        if (fBudgetFinalization) {
            ...
            // Note: there are still old valid budgets out there, but the check for the new 5 HLIX finalization collateral
            //       will also cover the old 50 HLIX finalization collateral.
...
void CBudgetManager::FillBlockPayee(CMutableTransaction& txNew, CAmount nFees, int algo)
{
    LOCK(cs);
    ...
    if (fDebug) LogPrintf("CBudgetManager::FillBlockPayee: got block value %lld\n",blockValue);
