...
public:
  ...
  BoostBigNum subsidyScalingFactor;
    ...
    {
      LogPrintf("GetBlockPoWHash: do GetBlockHeader()\n");
  ...
  inline bool update_ssf_next() const
  {
    return (nVersion & BLOCK_VERSION_UPDATE_SSF) && onCEM();
  }

    int GetBlockAlgo() const
  {
    CBlockHeader block;
    block.nVersion = nVersion;
  ...
  bool onFork() const {
    return this->nHeight >= Params().MultiPow_StartHeight();
  }

  bool onForkNext() const {
    return this->nHeight + 1 >= Params().MultiPow_StartHeight();
  }

  bool onCEM() const {
    return onFork() && this->nHeight >= Params().CEM_StartHeight();
  }
  
  bool onCEMNext() const {
    return onForkNext() && this->nHeight + 1 >= Params().CEM_StartHeight();
  }
  ...
  BoostBigNum GetBlockWorkBoost() const {
    ...
    if (bnTarget <=0)
      return 0;
    unsigned int algo_weight = GetAlgoWeight(this->GetBlockAlgo());
...
public:
	...
	bool onFork = this->onFork();
	if (this->GetBaseVersion()>4 && this->IsAuxpow() && onFork) {
	  if (ser_action.ForRead()) pauxpow.reset(new CAuxPow());
	  assert(pauxpow);
	  (*pauxpow).parentBlock.isParent = true;
	  int algo = CBlockIndex::GetBlockAlgo();
