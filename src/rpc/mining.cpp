...
UniValue GetNetworkHashPS(int lookup, int height, int algo)
{
    ...
    uint256 hashes_bn = GetBlockProof(*pb0);
    for (int i = 0; i < lookup; i++) {
        pb0 = pb0->pprev;
	if (!pb0) break;
        if (pb0->GetBlockAlgo()!=algo) {
          lookup++;
          continue;
        }
    ...
    return (hashes_bn.getdouble() / ((double)timeDiff));
}
...
UniValue getnetworkhashps(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
	    ...
	    "3. algo (numeric, optional, default=0)"
            "\nResult:\n"
...
UniValue getgenerate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or helix.conf setting gen)\n"
...
UniValue getminingalgo(const UniValue& params, bool fHelp)
{
  if (fHelp || params.size() != 0)
    throw runtime_error("getminingalgo"
			"\nGet the mining algorithm\n"
			"\nResult\n"
			"x (0-2) The number representing the mining algorithm\n"			
			);
...
UniValue setgenerate(const UniValue& params, bool fHelp)
{
    ...
    if (fGenerate && (chainActive.Height() >= Params().LAST_POW_BLOCK() && chainActive.Height() + 1 < Params().MultiPow_StartHeight()))
...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    ...
    if (vNodes.empty() && Params().NetworkID() != CBaseChainParams::REGTEST)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Helix is not connected!");

    if (IsInitialBlockDownload() && Params().NetworkID() != CBaseChainParams::REGTEST)
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Helix is downloading blocks...");
...
UniValue getauxblock(const UniValue& params, bool fHelp)
{
  ...
  if (params.size() < 2) {
    ...
    if (params.size() == 1) {
      miningAlgoChosen = atoi(params[0].get_str());
      LogPrintf("miningAlgoChosen = %d\n",miningAlgoChosen);
    }
...

    {
	  ...
	  || (mempool.GetTransactionsUpdated() != nTransactionsUpdatedLast
	      && GetTime() - nStart > 60)) {
	if (pindexPrev != chainActive.Tip()) {
	  LogPrintf("getauxblock: clear mapNewBlock\n");
	  mapNewBlock.clear();
	...
	pblocktemplate = CreateNewBlockWithKey(reservekey,pwalletMain,false,miningAlgoChosen);
	if (!pblocktemplate)
	  throw JSONRPCError(RPC_OUT_OF_MEMORY, "out of memory");
	...
	LogPrintf("getauxblock: set mapNewBlock[%s]\n",pblock->GetHash().ToString());
...
UniValue result(UniValue::VOBJ);
    ...
    */
    ...
    //result.push_back(Pair("address",CBitcoinAddress(address).ToString()));
  ...
  if (strlen(hash_str)>0 && mit == mapNewBlock.end()) {
    LogPrintf("getauxblock: block hash unknown\n");
  ...
  CValidationState state;
  LogPrintf("getauxblock do ProcessNewBlock\n");
  bool fAccepted = ProcessNewBlock(state, NULL, &block);
  if (!fAccepted) {
    LogPrintf("getauxblock: rejected\n");
    return "rejected";
  }
