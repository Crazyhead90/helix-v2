...
void UpdateTime(CBlockHeader* pblock, const CBlockIndex* pindexPrev)
{
    ...
    if (Params().AllowMinDifficultyBlocks())
      pblock->nBits = GetNextWorkRequired(pindexPrev, pblock->GetAlgo());
}

std::pair<int, std::pair<uint256, uint256> > pCheckpointCache;
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake, int algo)
{
    ...
    if (chainActive.Height() + 1 >= Params().MultiPow_StartHeight()) {
	LogPrintf("prev height + 1 >= multipow_startheight\n");
	if (fProofOfStake) {
	    pblock->SetAlgo(ALGO_POS);
	}
	else {
	    LogPrintf("pblock setalgo %d\n",algo);
	    LogPrintf("pblock->nVersion before = %d\n",pblock->nVersion);
	    pblock->SetAlgo(algo);
	    LogPrintf("pblock->nVersion after = %d\n",pblock->nVersion);
	}
...

    {
        ...
        for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (tx.IsZerocoinSpend()) {
                    ...
                    //Priority = (age^6+100000)*amount - gives higher priority to zhlixs that have been in mempool long
                    //and higher priority to zhlixs that are large in value
                    ...
                    // zHLIX spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zHlix spends in this block or tx
            if (tx.IsZerocoinSpend()) {
                int nHeightTx = 0;
                ...
                //This zHLIX serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
	  ...
	  FillBlockPayee(txNew, nFees, algo);
	...
	if (chainActive.Height() + 1 >= Params().CEM_StartHeight()) {
	  //LogPrintf("miner on fork\n");
	  CBlockIndex * pprev_algo = pindexPrev;
	  if (pprev_algo->GetBlockAlgo()!=algo) {
	    pprev_algo = get_pprev_algo(pindexPrev,algo);
	  }
	  if (!pprev_algo) { // todo: check
	...
	LogPrintf("do GetNextWorkRequired\n");
        pblock->nBits = GetNextWorkRequired(pindexPrev, pblock->GetAlgo());
        pblock->nNonce = 0;
        if (fZerocoinActive) {
            ...
            if (nHeight >= pCheckpointCache.first || pCheckpointCache.second.first != hashBlockLastAccumulated) {
                //For the period before v2 activation, zHLIX will be disabled and previous block's checkpoint is all that will be needed
        ...
        CValidationState state;
	LogPrintf("do TestBlockValidity\n");
...
CBlockTemplate* CreateNewBlockWithKey(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake, int algo)
{
    CPubKey pubkey;
    ...
    static int nFirstMultiPowBlock = Params().MultiPow_StartHeight();

    // If we're building a late PoW block, don't continue
    if ((nHeightNext > nLastPOWBlock) && (nHeightNext < nFirstMultiPowBlock) && !fProofOfStake) {
    ...
    return CreateNewBlock(scriptPubKey, pwallet, fProofOfStake, algo);
}
...
void BitcoinMiner(CWallet* pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        if (fProofOfStake) {
	  LogPrintf("fProofOfStake\n");
            if (chainActive.Tip()->nHeight < Params().LAST_POW_BLOCK()) {
	      LogPrintf("chainActive.Tip() < last_pow_block\n");
	      ...
	      if (pwallet->IsLocked()) LogPrintf("pwallet->IsLocked\n");
	      if (!fMintableCoins) LogPrintf("!fMintableCoins\n");
	      if (pwallet->GetBalance()>0 && nReserveBalance >= pwallet->GetBalance()) LogPrintf("nReserveBalance >= pwallet->GetBalance()\n");
	      if (!masternodeSync.IsSynced()) LogPrintf("!masternodeSync.IsSynced()\n");
            ...
            if (mapHashedBlocks.count(chainActive.Tip()->nHeight)) //search our map of hashed blocks, see if bestblock has been hashed yet
            {
                ...
                {
		  LogPrintf("GetTime()-mapHashedBlocks < max...\n");
                    MilliSleep(5000);
                    continue;
                }
            }
        } else { // PoW
	    if ((chainActive.Tip()->nHeight - 6) > Params().LAST_POW_BLOCK() && (chainActive.Tip()->nHeight+1)<Params().MultiPow_StartHeight())
             {
        ...
        // Create new block
        //
	LogPrintf("do gettransactionsupdated\n");
        unsigned int nTransactionsUpdatedLast = mempool.GetTransactionsUpdated();
        CBlockIndex* pindexPrev = chainActive.Tip();
        if (!pindexPrev) {
	  LogPrintf("!pindexPrev\n");
            continue;
	}

	LogPrintf("do CreateNewBlockWithKey\n");
        unique_ptr<CBlockTemplate> pblocktemplate(CreateNewBlockWithKey(reservekey, pwallet, fProofOfStake));
	LogPrintf("did CreateNewBlockWithKey\n");
        if (!pblocktemplate.get())
            continue;
	LogPrintf("pblocktemplate is good\n");

        CBlock* pblock = &pblocktemplate->block;
        IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);
	LogPrintf("did incrementextranonce\n");
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("HelixMiner() exception");
    } catch (...) {
        LogPrintf("HelixMiner() exception");
    }

    LogPrintf("HelixMiner exiting\n");
}
