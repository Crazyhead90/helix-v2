...
void CMasternodeSync::Process()
{
	...
	LogPrintf("SYNC_INITIAL -> GetNextAsset\n");
	GetNextAsset();
    }
    ...
    for (CNode* pnode : vNodes) {
	  ...
	  if (fDebug) LogPrintf("SYNC_SPORKS\n");
            if (pnode->HasFulfilledRequest("getspork")) continue;
	    if (fDebug) LogPrintf("!hasfulfilledrequest getspork attempt %d\n",RequestedMasternodeAttempt);
            pnode->FulfilledRequest("getspork");

            pnode->PushMessage("getsporks"); //get current network sporks
            if (RequestedMasternodeAttempt >= 2) {
	      if (fDebug) LogPrintf("SYNC_SPORKS -> GetNextAsset()\n");
		GetNextAsset();
	    }
            RequestedMasternodeAttempt++;
	...
	if (fDebug) LogPrintf("pnode->nVersion = %d, minmasternodepaymentsproto = %d\n",pnode->nVersion,masternodePayments.GetMinMasternodePaymentsProto());

        if (pnode->nVersion >= masternodePayments.GetMinMasternodePaymentsProto()) {
            if (RequestedMasternodeAssets == MASTERNODE_SYNC_LIST) {
	      if (fDebug) LogPrintf("SYNC_LIST\n");
	      if (fDebug) LogPrintf("CMasternodeSync::Process() - lastMasternodeList %lld (GetTime() - MASTERNODE_SYNC_TIMEOUT) %lld RequestedMasternodeAttempt %d\n", lastMasternodeList, GetTime() - MASTERNODE_SYNC_TIMEOUT,RequestedMasternodeAttempt);
		  ...
		  if (fDebug) LogPrintf("SYNC_LIST -> GetNextAsset\n");
                    GetNextAsset();
                    return;
                }

                if (pnode->HasFulfilledRequest("mnsync")) continue;
		if (fDebug) LogPrintf("!pnode->HasFulfilledRequest\n");
		      ...
		      if (fDebug) LogPrintf("CMasternodeSync::Process - ERROR - Sync has failed, will retry later\n");
                        RequestedMasternodeAssets = MASTERNODE_SYNC_FAILED;
                        RequestedMasternodeAttempt = 0;
                        lastFailure = GetTime();
                        nCountFailures++;
                    } else {
		      if (fDebug) LogPrintf("SYNC_LIST 2 -> GetNextAsset\n");
                        GetNextAsset();
                    }
                    return;
                }
	      ...
	      if (fDebug) LogPrintf("SYNC_MNW lastMasetnodeWinner %d RequestedMasternodeAttempt %d\n",lastMasternodeWinner,RequestedMasternodeAttempt);
		  ...
		  if (fDebug) LogPrintf("SYNC_MNW -> GetNextAsset\n");
                    GetNextAsset();
                    return;
                }

                if (pnode->HasFulfilledRequest("mnwsync")) continue;
		if (fDebug) LogPrintf("!pnode->HasFulfilledRequest, RequestedMasternodeAttempt = %d\n",RequestedMasternodeAttempt);
		      ...
		      if (fDebug) LogPrintf("SYNC_MNW 2 -> GetNextAsset()\n");
                        GetNextAsset();
                    }
                    return;
                }
        ...
        if (pnode->nVersion >= ActiveProtocol()) {
	      ...
	      if (fDebug) LogPrintf("SYNC_BUDGET\n");
		  ...
		  if (fDebug) LogPrintf("SYNC_BUDGET -> GetNextAsset()\n");
                    GetNextAsset();
		  ...
		  if (fDebug) LogPrintf("SYNC_BUDGET 2 -> GetNextAsset\n");
                    GetNextAsset();
                    activeMasternode.ManageStatus();
                    return;
                }
