...
public:
  ...
  bool IsTransactionValid(const CTransaction& txNew, CBlockIndex* pindexPrev, int algo);
...
public:
    ...
    bool IsTransactionValid(const CTransaction& txNew, const CBlock& block);
    ...
    void ProcessMessageMasternodePayments(CNode* pfrom, std::string& strCommand, CDataStream& vRecv);
    std::string GetRequiredPaymentsString(int nBlockHeight);
  void FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, int algo);
