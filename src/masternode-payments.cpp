...
bool IsBlockPayeeValid(const CBlock& block, int nBlockHeight)
{
    ...
    const CTransaction& txNew = (nBlockHeight > Params().LAST_POW_BLOCK() && nBlockHeight < Params().MultiPow_StartHeight() ? block.vtx[1] : block.vtx[0]);
    ...
    if (masternodePayments.IsTransactionValid(txNew, block))
        return true;
...
void FillBlockPayee(CMutableTransaction& txNew, CAmount nFees, int algo)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    if (!pindexPrev) return;

    if (IsSporkActive(SPORK_13_ENABLE_SUPERBLOCKS) && budget.IsBudgetPaymentBlock(pindexPrev->nHeight + 1)) {
        budget.FillBlockPayee(txNew, nFees, algo);
    } else {
        masternodePayments.FillBlockPayee(txNew, nFees, algo);
    }
}
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, int algo)
{
  bool fProofOfStake = algo == ALGO_POS;
    CBlockIndex* pindexPrev = chainActive.Tip();
    if (!pindexPrev) return;
    ...
    CAmount blockValue = GetBlockValue(pindexPrev,algo);
    if (fDebug) LogPrintf("CMasternodePayments::FillBlockPayee got block value %lld\n",blockValue);
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew, CBlockIndex* pindexPrev, int algo)
{ // todo: check
    LOCK(cs_vecPayments);
    ...
    CAmount nReward = GetBlockValue(pindexPrev,algo);
...
bool CMasternodePayments::IsTransactionValid(const CTransaction& txNew, const CBlock& block)
{
  ...
  if (mi != mapBlockIndex.end() && (*mi).second) {
    ...
    nBlockHeight = pindexPrev->nHeight; // todo check if pindex will always exist. What if at tip?
  }
  else {
    LogPrintf("CMasternodePayments::IsTransactionValid: can't find pindexPrev, so returning true\n");
    return true;
  }
  LOCK(cs_mapMasternodeBlocks);
  if (mapMasternodeBlocks.count(nBlockHeight)) {
    return mapMasternodeBlocks[nBlockHeight].IsTransactionValid(txNew,pindexPrev,algo);
  }
  return true;
}
